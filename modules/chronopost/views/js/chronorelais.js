...
function createAllPointRelais(json) {
    ...
    if (data.hasOwnProperty("listePointRelais") && data.listePointRelais) {
        ...
        let chronoRelaisSelect = $('input[name="chronorelaisSelect"]');

        chronoRelaisSelect.change(function () {
        ...
        // Open closest marker
        openMarker(chronoRelaisSelect.val());
...
function cleanContainers() {
    ...
    if (checkedCarrier === CHRONORELAIS_AMBIENT_ID || checkedCarrier === CHRONORELAIS_ID || checkedCarrier === RELAISEUROPE_ID ||
        ...
        if (container.length === 0) {
            $('#chronorelais_container').detach().insertAfter($('#extra_carrier'));
        } else {
            $('#chronorelais_container').detach().appendTo(container);
        }
    }
}
...
$(function () {
    ...
    if (typeof CHRONORELAIS_AMBIENT_ID === 'undefined' && typeof CHRONORELAIS_ID === 'undefined' &&
        ...
        typeof TOSHOPDIRECT_ID === 'undefined') {
        return false;
    }
    ...
    body.on('click', '.delivery_options span.custom-radio > input[type=radio], input[name*="delivery_option"]', function (e) {
        ...
        if (value === parseInt(CHRONORELAIS_AMBIENT_ID) || value === parseInt(CHRONORELAIS_ID) ||
            ...
            value === parseInt(TOSHOPDIRECT_ID)) {
